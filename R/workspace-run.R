...
accu_workspace_run <- function(client = NULL) {
  ...
  library(subprocess)
  handle <- spawn_process('/usr/local/bin/R', c('CMD', 'BATCH', paste('--args', args), paste0('--workspaceRunId=', argv$workspaceRunId), argv$codePath, paste0('/tmp/output_', argv$workspaceRunId, '.log')))
